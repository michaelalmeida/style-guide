...
import {GlobalStyles, ResultTitle, FontSample, ColorCard  } from './ResultStyles';
...
const Result = props => {
    ...
    return (
        <>
            <GlobalStyles primaryColor={props.colors['first'].hex} secondaryColor={props.colors['second'].hex} primaryFont={props.typography[0]} secondaryFont={props.typography[1]}/>
            <ResultTitle primaryColor={props.colors["first"].hex}>
                <input value={guideName} className="primary-font" onChange={nameHandler}/>
            </ResultTitle>
            <Title style={{marginTop: '50px'}}>Typography</Title>
            {props.typography.length > 0 ? props.typography.map((font, index) => {
                return <div className={index === 0 ? 'primary-font' : 'secondary-font'} key={index}>
            ...
            {Object.entries(props.colors).map(color => {
                return <Col span={6} key={color[0]}>
                        <ColorCard background={color[1].hex}></ColorCard>
                        <p>{color[1].hex}</p>
                        <p>rgb: {`${color[1].rgba.r}, ${color[1].rgba.g}, ${color[1].rgba.b}`}</p>
                    </Col>
            })}
            </Row>
            {props.elements.length > 0 ? <Title style={{marginTop: "50px"}}>Elements</Title> : ''}
            {props.elements.map(element => {
                return <Row gutter={16} key={element} style={{marginBottom : '25px'}}>
